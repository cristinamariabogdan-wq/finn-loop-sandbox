...
Using the Linear connector, list issues on team `CRI` that meet every
condition:
...
- Fetch the latest default branch from `origin` and create or resume a branch
  named `CRI-NNN-short-slug`, using the issue's real identifier.
...
- `Closes CRI-NNN`, using the real Linear issue identifier
